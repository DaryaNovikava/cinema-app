...
export function fetchMoviesData<T>(
  url: string,
...
export function useMoviesData<T>(url: string, schema: z.ZodSchema<T>) {
  return useQuery<T>({
    queryKey: [url],
...
export async function addMovieToFavorites(movieId: number): Promise<object> {
  try {
...
export async function getUserFavorites(): Promise<MovieList> {
  try {
    const response = await fetch(`${API_URL}favorites`, {
      method: 'GET',
      credentials: 'include',
